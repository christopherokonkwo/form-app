...
class StatsAggregator
{
    ...
    protected function calculateReadTime(?string $text): string
    {
        ...
        return sprintf(
            '%d %s %s',
            $minutes,
            Str::plural(trans('app.min', [], optional($this->user)->locale), $minutes),
            trans('app.read', [], optional($this->user)->locale)
        );
    }
    ...
    protected function calculateTopReferers(Post $post): array
    {
        ...
        $collection = new Collection();
        $data->each(function ($item, $key) use ($collection) {
            if (empty(parseReferer($item->referer))) {
                $collection->push(trans('::app.other', [], $this->user->locale));
            } else {
                $collection->push(parseReferer($item->referer));
            }
        });
